    ...
    def process_images(self, image_paths, output_path, is_preview=False):
        ...
        for i, img_path in enumerate(final_image_paths):
            ...
            try:
                ...
                img_fitted = ImageOps.fit(img, (slot["w"], slot["h"]), Image.Resampling.LANCZOS)
        ...
        if actual_frame_path:
            logger.debug(f"Frame found at: {actual_frame_path}. Size: {os.path.getsize(actual_frame_path)} bytes")
            frame = Image.open(actual_frame_path).convert("RGBA")
            frame = frame.resize(canvas_size, Image.Resampling.LANCZOS)
            # Paste frame (RGBA) on top of images (RGB) using itself as mask
        ...
        if is_preview:
            logger.info(f"Preview collage with Mock QR saved to {output_path}")
        else:
